...
//
// ── 판정 박제 (aiwait-mouse-7f4c1a08 · 2026-08-31) — «그대로 둔다» ──────────
//
// 물음 2. 마법사 네 화면이 쓰는 공용 대기 표시(AiWaitBar)로 여기도 통일하는가.
// 답    . 통일하지 않는다. 아래 판정을 뒤집기 전에 이 문단을 먼저 읽어라.
//
// 사용자가 지금 «잃고 있는 것» 이 없다. 이 창은 기다리는 동안 이미 셋을 준다 —
//   (1) 무슨 일을 하는 중인지 : 액션 이름 + 「AI 가 분석 중」
//   (2) 살아 있다는 증거     : 0.25초마다 갱신되는 경과 시간
//   (3) 마우스로 멈추는 법   : 「취소」 버튼 (StreamView 아래쪽)
// 남은 차이는 셋뿐이다 — 경과 시간을 5초 뒤부터 보일 것, 「언제 저절로 멈추는지」
// 한 줄, 버튼 이름을 「그만두기」로. 셋 다 문면이지 «잃은 기능» 이 아니다.
//
// 그리고 셋 다 이 파일이 아니라 StreamView.tsx 안에 있다. 이 파일이 할 수 있는
// 일은 StreamView «위에» 대기 막대를 하나 더 얹는 것뿐인데, 그러면 한 창 안에
// 멈추는 버튼이 둘(「그만두기」·「취소」)이고 경과 시간이 둘이 된다. 컴퓨터
// 용어를 모르는 사용자에게 그것은 통일이 아니라 «어느 쪽이 진짜인가» 라는 새
// 물음이다 — 지금보다 나빠진다. 그래서 손대지 않는다.
//
// 통일하려면 고칠 자리는 하나다 — StreamView.tsx 의 footer(문면 3곳). 이 파일이
// 아니다. 그때는 SelectionPopover 등 StreamView 를 쓰는 모든 화면이 함께 바뀐다.
//
// 한 가지 더, 여기의 「취소」는 마법사의 「그만두기」와 «뜻이 다르다» —
// handleCancel 은 호출을 멈추고 창까지 닫는다(onComplete("discard")). 마법사의
// 그만두기는 멈추기만 하고 화면은 그대로 남는다. 낱말을 같게 만들려면 이 차이를
// 먼저 정해야 한다. 문면만 바꾸면 같은 이름이 서로 다른 일을 하게 된다.
