...
export const CONNECTION_VIEW_TYPE = "zettel-connect-panel";

export class ConnectionPanel extends ItemView {
  private plugin: ZettelConnectPlugin;
  ...
  constructor(leaf: WorkspaceLeaf, plugin: ZettelConnectPlugin) {
    super(leaf);
    this.plugin = plugin;
  }
  ...
  async recommendForActive(): Promise<void> {
    ...
    try {
      ...
      console.error("[zettel-connect]", err);
      new Notice(
  ...
  private async renderEmpty(message?: string): Promise<void> {
    ...
    c.empty();
    c.addClass("zc-panel");
    c.createEl("h3", { text: "Zettel Connect" });
  ...
  private async renderCandidates(
    ...
    header.createEl("h3", { text: "Zettel Connect" });
  ...
  private async insertLink(cand: Candidate): Promise<void> {
    ...
    try {
      ...
      console.warn("[zettel-connect] frontmatter update failed:", err);
